...
### Вариант А: Через админ-панель Bitrix24 (рекомендуется)

1. Перейдите в админ-панель Bitrix24:
   ```
   https://ваш-домен.ru/bitrix/admin/trustme_sign_webhook_setup.php
   ```
   
   Или через меню: **Настройки** → **Trust Me Sign** → **Настройка Webhook**
2. Укажите:
   - **API Token**: ваш токен API
   - **Test Mode**: включите для тестового режима
   - **Webhook URL**: URL вашего webhook (автоматически определяется)
3. Нажмите кнопку **"Установить webhook"**
4. После успешной установки вы увидите сообщение об успехе

### Вариант Б: Через API (программно)

Если вы хотите настроить webhook программно, создайте временный скрипт:

**Где разместить скрипт:**

1. **Рекомендуемый вариант**: Создайте папку `/local/scripts/` (если её нет) и поместите туда скрипт:
   ```
   /local/scripts/setup_webhook.php
   ```

2. **Альтернативный вариант**: Временно поместите в корень сайта:
   ```
   /setup_webhook.php
   ```
   ⚠️ **Важно**: После выполнения скрипта обязательно удалите его из корня сайта!
...
1. **Через браузер**: Откройте `https://ваш-домен.ru/local/scripts/setup_webhook.php` (или `/setup_webhook.php` если в корне)
2. **Через командную строку** (если есть доступ к серверу):
   ```bash
   php /path/to/site/local/scripts/setup_webhook.php
   ```

**После выполнения:**
- Удалите скрипт из корня сайта (если размещали там)
- Или оставьте в `/local/scripts/` для будущего использования

### Вариант В: Через админку TrustMe (если доступна)
...
1. **Вариант 1**: Хранить `contract_id` в дополнительном поле сделки
...
2. **Вариант 2**: TrustMe передаёт `deal_id` в webhook
